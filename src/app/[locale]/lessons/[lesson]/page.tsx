...
type Params = Promise<{ locale: string; lesson: string }>;
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const lesson = getLesson(locale, lessonSlug);
  if (!lesson) return {};

  const path = `/lessons/${lessonSlug}`;
  return {
    title: lesson.title,
    description: lesson.summary,
    keywords: lesson.tags.map((key) => tagLabel(locale, key)),
    alternates: {
...
export default async function LessonPage({ params }: { params: Params }) {
  ...
  const { module: primary, previous, next } = getAdjacentLessons(locale, lessonSlug);
  const owners = getModulesForLesson(locale, lessonSlug);
  const alsoIn = owners.filter((entry) => entry.slug !== primary?.slug);
    ...
    <div className="mx-auto max-w-6xl px-5 py-12">
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_15rem] lg:gap-12">
        <article className="min-w-0">
          {primary ? (
            <Link
              href={`/${locale}/learn/${primary.slug}`}
